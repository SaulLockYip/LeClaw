...
function Sidebar() {
  ...
  return (
    <div className="w-60 bg-black flex flex-col">
      ...
      {/* COMPANY Section */}
      <div className="px-4 py-2">
        <span className="text-xs font-semibold text-white/50 uppercase tracking-wider">Company</span>
      </div>
          ...
          return (
            <div key={dept.id} className="mb-3">
              <NavLink
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded text-sm text-white/70 hover:bg-white hover:text-black ${
                  isPathActive(`/departments/${dept.id}`) ? 'bg-white/20 text-white' : ''
                }`
                }
              >
              ...
              {staff.length > 0 && (
                <div className="ml-8 space-y-1">
                  {staff.map((s) => (
                    <div key={s.id} className="flex items-center gap-2 px-3 py-0.5 text-white/50 text-xs">
